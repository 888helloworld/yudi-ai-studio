...
当前画面工坊页面只保留 2 个经过真实验证、请求和输出严格一致的尺寸入口。服务端保存图片后会读取真实像素并写入历史记录。
...
尺寸参数原样传给上游，服务端不缩放、不裁切、不补白。返回尺寸与请求不一致时任务失败并退款。

服务端不会缩放、裁切或补白。上游返回尺寸与请求不一致时，任务失败并退款。
...
- 公网注册是否仍需要邀请码。
- 画面工坊能打开。
- 尺寸按钮只显示 `1024x1536` 和 `1536x1024` 两个实测原生规格。
- 管理后台能登录。
- 上游 ping 可用。
