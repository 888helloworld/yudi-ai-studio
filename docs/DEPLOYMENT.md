...
当前画面工坊页面只开放实测可用的方图规格，服务端保存图片后会读取真实像素并写入历史记录。`gpt-image-2` 官方显式尺寸约束是：宽高为 16 的倍数，比例在 1:3 到 3:1 之间，且不超过 3840x2160 等量像素；但当前接入上游实际会忽略多种请求尺寸。
