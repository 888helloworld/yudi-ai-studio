...
2026-09-05：更新前先执行 `npm run backup:full`，数据库与图片一起校验。Windows 每日完整备份安装方法、恢复步骤和当前部署边界见 [本轮记录](RELEASE_2026-09-05.md)。公网发布脚本新增生产配置检查；本机开发不需要伪造域名通过检查。

这份文档讲一件事：怎么把“雨滴AI”更新到服务器，同时不把用户、积分、历史、图片这些运行数据弄丢。
...
当前画面工坊页面保留 5 个实测尺寸入口，服务端保存图片后会读取真实像素并写入历史记录。`gpt-image-2` 官方显式尺寸约束是：宽高为 16 的倍数，比例在 1:3 到 3:1 之间，且不超过 3840x2160 等量像素；页面按 xixu 实测输出展示常用规格。
...
| 高清横图 | `1672x941` | `2048x1152` |
| 高清竖图 | `941x1672` | `1152x2048` |

2026-06-29 全量复测：1K 竖图、1K 横图可严格对齐；方图实际为 `1254x1254`；2K/4K 横竖会落到 xixu 的实际高清规格 `1672x941` / `941x1672`。页面使用 2K 请求拿同等实际输出，避免用 4K 请求误导用户。
