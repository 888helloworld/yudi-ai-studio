...
当前画面工坊页面保留 4 个常用尺寸入口，服务端保存图片后会读取真实像素并写入历史记录。`gpt-image-2` 官方显式尺寸约束是：宽高为 16 的倍数，比例在 1:3 到 3:1 之间，且不超过 3840x2160 等量像素；页面按本项目历史实测展示常用规格。
...
2026-06-29 复核历史记录：`#652` 为 `1024x1536` 竖图，`#653` 为 `1536x1024` 横图，`#654` 为 `2560x1440 -> 1672x941`。实际输出继续以 `output_dimensions` 为准。
