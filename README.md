...
冰侠客是一个面向内容创作者、电商运营、设计团队和商家用户的 AI 图文创作工具。它不只服务小红书场景，也覆盖通用 AI 生图、商品主图、模特穿搭图、图文内容生产、提示词反推、历史资产管理和积分运营。
...
服务端还有独立队列 `XI_XU_MAX_ACTIVE_JOBS`，设置为 `0` 表示不限制服务端同时运行的 Xi-Xu 图片任务。
...
- `xi-image.html`：冰侠客 · gpt-image-2 生图、参考图改图、看图写 Prompt、生成记录。
...
- 服务端 `XI_XU_MAX_ACTIVE_JOBS` 控制真正同时运行的 Xi-Xu 图片任务。
